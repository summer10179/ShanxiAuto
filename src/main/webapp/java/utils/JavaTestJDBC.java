...
public class JavaTestJDBC {

     public Map ChaRu() throws ClassNotFoundException, SQLException {
        ...
        String PERIOD=null;
        while(rs.next()){
            TOPIC_CODE=rs.getString("TOPIC_CODE");
            PERIOD=rs.getString("PERIOD");
        }

        Map  map=new HashMap();
        map.put("TOPIC_CODE",TOPIC_CODE);
...
//             TOPIC_CODE=rs.getString("TOPIC_CODE");
//             PERIOD =rs.getString("PERIOD");
//             System.out.println( TOPIC_CODE+ PERIOD );
//         }
//         Map res=new HashMap();
//         res.put("TOPIC_CODE",TOPIC_CODE);
...
//        return  res;res
    ...
    public  void Shanchu() throws ClassNotFoundException, SQLException {
        ...
        Connection conn = DriverManager.getConnection(url, user, password);
        //3.通过数据库的连接操作数据库，实现增删改查（使用Statement类）
        String s = "delete from P_CHECK_CONS_DTL where  DET_DATA_ID=5611111812";
        PreparedStatement pst = conn.prepareStatement(s);
